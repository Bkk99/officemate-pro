...
import { CHAT_ROOMS_SAMPLE, DEPARTMENTS } from '../../constants';
...
// Specific icon for the empty state from user's screenshot (approximated)
...
export const ChatPage: React.FC = () => {
  const { user } = useAuth();
  const [chatRooms] = useState<ChatRoom[]>(CHAT_ROOMS_SAMPLE.map(r => ({...r, memberIds: MOCK_USERS.map(u=>u.id) }))); 
  ...
  useEffect(() => {
    const handleNewChatMessage = (incomingMessage: ChatMessage) => {
        setMessages(prev => {
            const roomMessages = prev[incomingMessage.roomId] || [];
            if (roomMessages.find(m => m.id === incomingMessage.id)) {
                return prev;
            }
            return {
                ...prev,
                [incomingMessage.roomId]: [...roomMessages, incomingMessage],
            };
        });

        if (incomingMessage.roomId !== activeRoomId) {
            setUnreadRoomIds(prev => new Set(prev).add(incomingMessage.roomId));

            const senderUser = MOCK_USERS.find(u => u.id === incomingMessage.senderId);
            const targetRoomDetails = chatRooms.find(r => r.id === incomingMessage.roomId);

            if (senderUser && targetRoomDetails) {
                 const isSystemMessage = incomingMessage.senderId === 'system-payroll';
                 const isExecutiveSender = senderUser.role === UserRole.ADMIN;
                 const isHrSenderInHrRoom = incomingMessage.roomId === HR_ANNOUNCEMENT_ROOM_ID && senderUser.department === DEPARTMENTS.find(d => d === "ฝ่ายบุคคล");

                if (isSystemMessage || isExecutiveSender || isHrSenderInHrRoom) {
                    let toastMsg = incomingMessage.text || "คุณมีข้อความใหม่";
                    if (isExecutiveSender) {
                        toastMsg = `ประกาศจากผู้บริหาร (${incomingMessage.senderName}) ในห้อง "${targetRoomDetails.name}"`;
                    } else if (isHrSenderInHrRoom) {
                         toastMsg = `ประกาศใหม่ในห้อง "${targetRoomDetails.name}"`;
                    } else if (isSystemMessage) {
                        toastMsg = `ข้อความจาก ${incomingMessage.senderName} ในห้อง "${targetRoomDetails.name}"`;
                    }
                     setToastInfo({ message: toastMsg, roomId: incomingMessage.roomId, roomName: targetRoomDetails.name });
                     if(toastTimeoutRef.current !== null) clearTimeout(toastTimeoutRef.current);
                     toastTimeoutRef.current = window.setTimeout(() => { setToastInfo(null); toastTimeoutRef.current = null; }, 5000);
                }
            }
        }
    };

    const unsubscribe = subscribeToChatNotifications(handleNewChatMessage);
    return () => unsubscribe();
  }, [activeRoomId, chatRooms]);

  useEffect(() => {
    const count = unreadRoomIds.size;
    updateGlobalChatUnreadCount(count);
  }, [unreadRoomIds]);
    ...
    <div className="flex flex-col h-[calc(100vh-12rem)] md:h-[calc(100vh-10rem)] relative">
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              <li key={room.id}>
                ...
                >
                  <div className="flex items-center space-x-2 truncate">
                    <ChatBubbleLeftEllipsisIcon className="h-5 w-5 flex-shrink-0" />
        ...
        <div className="flex-1 flex flex-col bg-white">
          ...
          {(messages[activeRoomId] || []).length > 0 ? (
            <div className="flex-1 p-6 space-y-4 overflow-y-auto bg-gray-50">
                ...
                <div key={msg.id} className={`flex ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg shadow ${
                        ...
                        : msg.senderId === 'system-payroll'
                        ? 'bg-blue-100 text-blue-800 border border-blue-300'
                        : 'bg-secondary-200 text-secondary-800'
                    }`}>
                    {(msg.senderId !== user.id) && <p className="text-xs font-semibold mb-0.5">{msg.senderName}</p> }
                    <p className="text-sm whitespace-pre-wrap">{msg.text}</p>
                    <p className={`text-xs mt-1 ${
                        msg.senderId === user.id ? 'text-primary-200' 
                        : msg.senderId === 'system-payroll' ? 'text-blue-600'
                        : 'text-secondary-500'
                        } text-right`}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-10 bg-primary-50"> {/* Updated empty state background */}
                <GroupChatIcon className="w-20 h-20 text-primary-400 mb-6" /> {/* Updated icon, size and color */}
                <h3 className="text-xl font-semibold text-primary-600 mb-2">แชทภายในองค์กร</h3> {/* Updated text color */}
          ...
          <div className="bg-white border-t border-gray-200 p-4">
            <div className="flex items-center space-x-3">
              <Textarea
                ...
                placeholder={`ข้อความถึง #${activeRoomName}...`}
                rows={1}
              ...
              />
              <Button onClick={handleSendMessage} disabled={!newMessage.trim()} aria-label="ส่งข้อความ">
