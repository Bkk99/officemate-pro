...
export const getAllInventoryItems = async (category?: 'อุปกรณ์ IT' | 'General'): Promise<InventoryItem[]> => {
    ...
    return rows.map((r: InventoryItemRow) => ({ id: r.id, name: r.name, sku: r.sku, category: r.category, quantity: r.quantity, unitPrice: r.unit_price, supplier: r.supplier || undefined, lastUpdated: r.last_updated }));
};
...
export const addInventoryItem = async (itemData: Omit<InventoryItem, 'id' | 'lastUpdated'>): Promise<InventoryItem> => {
    const dbData: Database['public']['Tables']['inventory_items']['Insert'] = { name: itemData.name, sku: itemData.sku, category: itemData.category, quantity: itemData.quantity, unit_price: itemData.unitPrice, supplier: itemData.supplier || null };
    ...
    return { id: r.id, name: r.name, sku: r.sku, category: r.category, quantity: r.quantity, unitPrice: r.unit_price, supplier: r.supplier || undefined, lastUpdated: r.last_updated };
};
...
export const updateInventoryItem = async (updatedItem: InventoryItem): Promise<InventoryItem> => {
    ...
    const dbData: Database['public']['Tables']['inventory_items']['Update'] = { name: itemData.name, sku: itemData.sku, category: itemData.category, quantity: itemData.quantity, unit_price: itemData.unitPrice, supplier: itemData.supplier || null };
